...
function render (to, from, next) {
  ...
  .then(() => {
    ...
    return Promise.all(Components.map((Component, i) => {
      ...
      const context = getContext({ to, store, isClient: true, next: _next.bind(this), error: this.error.bind(this) })
      // Validate method
...
function hotReloadAPI (_app) {
  ...
  $nuxt.$forceUpdate = function () {
    ...
    const context = getContext({ route: router.currentRoute, store, isClient: true, next: next.bind(this), error: _app.error })
