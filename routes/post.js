...
routers.put('/:id/comments/:commentid/stars',(req,res) => {
    req.params.ip = req.headers['x-real-ip'] || req.connection.remoteAddress.split(":").pop();
    postSrv.stars_comment(req.params,success =>{
        res.status(200).send(success);
    },err =>{
        res.status(400).send(err);
    });
...
routers.put('/:id/comments/:commentid/subcomments/:subid/stars',(req,res) => {
    
    req.params.ip = req.headers['x-real-ip'] || req.connection.remoteAddress.split(":").pop();
    postSrv.stars_sub_comment(req.params,success =>{
        res.status(200).send(success);
    },err =>{
        res.status(400).send(err);
    });
