<template>
    <div class='panel panel-default'>
        <div class='panel-heading'>Preview</div><br />
                        ...
                        case "img":
                            ...
                            if (tags !== null){
                                tags.forEach(img =>{
                                    ...
                                    const alt = attrib.match(/a=\'(.*?)\'/) === null ? "":` alt='${(attrib.match(/a=\'(.*?)\'/))[1]}'`; //Replace the alt tag
                                    const w = attrib.match(/w=\'(.*?)\'/) === null ? "":` width='${(attrib.match(/w=\'(.*?)\'/))[1]}'`; //Replace the width tag
                                    const h = attrib.match(/h=\'(.*?)\'/) === null ? "":` height='${(attrib.match(/h=\'(.*?)\'/))[1]}'`; //Replace the height tag
