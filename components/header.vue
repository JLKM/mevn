<template>
    <div class='headerview'>
        <div class='row headerBtns'>
            <div class='col-md-9'>
                <nuxt-link class="navbar-text header-logo" to='/'><img src='/images/MEVNblank128x31.png' alt='MEVN stack' title='Powered by MongoDB, Express, Vue and Node/Nuxt'></nuxt-link>
                <ul class="nav navbar-nav header-link">
                    <span v-for='link in $store.state.headerLink' v-bind:data='link' v-bind:key='link.text' :class={dropdown:link.hasSub}>
                        ...
                        <div class="dropdown-content" v-if='link.hasSub'>
                            <nuxt-link v-for='sub in link.subLinks' v-bind:data='sub' v-bind:key='sub.text' :to='sub.url !== "" ? `${sub.url}/pages/1`:"javascript:void(0)"'>{{sub.name}}</nuxt-link>
                        </div>
                    </span>
                </ul>
