<template>
    <ul id='header_phone_navList'>
        <li v-for='link in headerLink' v-bind:data='link' v-bind:key='link.text'>
            ...
            <ul class='sub_Nav_List' v-if='link.hasSub' :id='subNavId'>
                <button class='navbtn' v-for='sub in link.subLinks' v-bind:data='sub' v-bind:key='sub.text' @click='onSubClick(sub.url)'><b>{{sub.name}}</b></button>
            </ul>
        </li>
