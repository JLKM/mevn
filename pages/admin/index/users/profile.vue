            ...
            <div class='row'>
                <div class='col-sm-3'>
                    <div class='input-group'>
                        <input class='form-control' placeholder="Search for username or email" v-model='userKyWrds'/>
                    ...
                    <ul class='nav nav-second-level user-search-result' v-if='users.length != 0'>
                        <a v-for="user in users"  v-bind:data='user' v-bind:key='user.text' class='list-group-item list-group-item-action' @click="select(user)" href='javascript:void(0)'>
                            {{ user.username }}
                        </a>
                    </ul>
                </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            search(){
                if (this.userKyWrds == ""){
                    this.users.push({username:"Please enter a keyword",_id:"err"});
                }else{
                    $.ajax({
                        ...
                        success: res => {
                            ...
                            if (res.result.length == 0 ){
                                this.users.push({username:"No matching users",_id:"err"});
                            }else{
                                this.users = res.result;
                            }
                        },
                        error: err => {
                            this.users.push({username:"An error occurred",_id:"err"});
                        }
                    });
                }
            },
